...
class _ChatListScreenState extends State<ChatListScreen> {
  ...
  void initState() {
    super.initState();
    context.read<ChatSalesCubit>().loadConversations();
  }
    ...
    return Scaffold(
      ...
      body: BlocBuilder<ChatSalesCubit, ChatSalesState>(
        builder: (context, salesState) {
          if (salesState.isLoading && salesState.conversations.isEmpty) {
            return const LoadingIndicator();
          }

          if (salesState.error != null && salesState.conversations.isEmpty) {
            return ErrorDisplay(
              message: salesState.error!,
              onRetry: () => context.read<ChatSalesCubit>().loadConversations(),
            );
          }

          if (salesState.conversations.isEmpty) {
          ...
          return ListView.builder(
            itemCount: salesState.conversations.length,
            itemBuilder: (context, index) {
              final conversation = salesState.conversations[index];
              return ListTile(
                leading: CircleAvatar(
                  backgroundImage: conversation.consumerAvatarUrl != null
                      ? NetworkImage(conversation.consumerAvatarUrl!)
                      : null,
                  child: conversation.consumerAvatarUrl == null
