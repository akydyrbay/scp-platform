...
export default function ManagerDashboardPage() {
  ...
  const backlogOrders = stats?.recentOrders?.slice(0, 4) || []
          ...
          <div className='flex flex-col gap-8 xl:flex-row xl:gap-[86px]'>
            ...
            <div className='flex w-full max-w-[203px] flex-col gap-6'>
              <div className='rounded-[24px] border border-[#E3E8EF] bg-white p-6 shadow-[0_20px_40px_rgba(15,23,42,0.06)]'>
                ...
                <p className='mt-4 text-3xl font-semibold text-neutral-900'>{stats?.totalOrders || 0}</p>
                <p className='mt-2 text-sm text-neutral-500'>{stats?.pendingOrders || 0} pending</p>
              </div>

              <div className='rounded-[24px] border border-[#E3E8EF] bg-white p-6 shadow-[0_20px_40px_rgba(15,23,42,0.06)]'>
                <p className='text-xs uppercase tracking-[0.3em] text-neutral-400'>Link Requests</p>
                <p className='mt-4 text-3xl font-semibold text-neutral-900'>{stats?.pendingLinkRequests || 0}</p>
                <p className='mt-2 text-sm text-neutral-500'>Awaiting approval</p>
              </div>

              <div className='rounded-[24px] border border-[#E3E8EF] bg-white p-6 shadow-[0_20px_40px_rgba(15,23,42,0.06)]'>
                <p className='text-xs uppercase tracking-[0.3em] text-neutral-400'>Low Stock</p>
                <p className='mt-4 text-3xl font-semibold text-neutral-900'>{stats?.lowStockItems || 0}</p>
                <p className='mt-2 text-sm text-neutral-500'>Items need restocking</p>
              </div>
            </div>
          </div>
          ...
          {backlogOrders.length > 0 && (
            <div className='mt-10 rounded-2xl border border-neutral-200 bg-white p-6 shadow-[0_20px_48px_rgba(15,23,42,0.06)]'>
              <header className='mb-6 flex flex-wrap items-center justify-between gap-4'>
                ...
                <a href='/manager/orders' className='rounded-xl border border-neutral-200 px-4 py-2 text-sm font-semibold text-neutral-600 transition hover:border-primary-200 hover:text-primary-500'>
                  View All Orders
                </a>
              </header>
                  ...
                  <tbody className='divide-y divide-neutral-100 text-sm text-neutral-700'>
                    {backlogOrders.map(order => (
                      <tr key={order.id} className='transition hover:bg-neutral-50'>
                        <td className='px-6 py-4 font-semibold text-neutral-900'>#{order.id.slice(-8)}</td>
                        <td className='px-6 py-4'>{order.consumerName || 'Unknown'}</td>
                        <td className='px-6 py-4'>
                          <span className={`rounded-full px-3 py-1 text-xs font-semibold text-white ${
                            order.status === 'pending' ? 'bg-sky-500' :
                            order.status === 'accepted' ? 'bg-amber-400' :
